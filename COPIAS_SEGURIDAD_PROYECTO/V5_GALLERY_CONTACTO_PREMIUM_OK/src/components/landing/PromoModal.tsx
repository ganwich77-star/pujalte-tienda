...
export function PromoModal({ promos, onClose, onOpenStore, onContact }: PromoModalProps) {
  ...
  const HeaderContent = () => (
    <div className="flex flex-col items-center text-center space-y-3 md:space-y-4">
      ...
      <p className="text-sm md:text-xl lg:text-2xl text-white/80 font-medium leading-tight max-w-xl">
        {current.subtitle}
      </p>
    </div>
  )
    ...
    <div className="flex flex-col items-center text-center">
      {current.action !== 'none' && (
        <button 
          ...
          className="group relative flex items-center gap-4 bg-white text-black px-8 md:px-10 py-4 md:py-6 rounded-2xl md:rounded-3xl font-black uppercase tracking-[0.2em] text-[10px] md:text-xs transition-all duration-300 shadow-[0_20px_50px_rgba(0,0,0,0.1)] md:shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:shadow-xl active:scale-95 overflow-hidden"
        >
          ...
          >
            ...
            <div className="hidden md:block relative w-full aspect-video bg-black rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20">
              ...
              <div className={cn(
                ...
                current.contentPosition?.includes('top') && "justify-start text-center items-center",
                current.contentPosition?.includes('bottom') && "justify-end text-center items-center",
                current.contentPosition === 'center' && "justify-center text-center items-center",
                !current.contentPosition && "justify-center text-center items-center"
              )}>
